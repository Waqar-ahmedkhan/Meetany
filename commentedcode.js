...
// main code with websocket and messaging and video call
...
// import { Subscription } from './models/Subscription.js';
// import { CallHistory } from './models/CallHistory.js';
// import { Message } from './models/Message.js'; // New import for message model
// import multer from 'multer';
// import crypto from 'crypto';
...
//     // If current user has a preSubscription, ensure user matches the planName gender
//     if (preSubscribe && preSubscribe.planName !== user.gender) continue;
		
//     const userSubscribe = await Subscription.findOne({ type: "filter", user_id: user.uniqueID });

//     // Check userSubscribe: If it exists, ensure currentUser matches the planName gender
...
// const users = [];
// let ongoingCalls = [];
// let activeChats = {}; // Store active chat sessions by call pairing

// // Function to save chat message to database
// const saveMessage = async (senderId, receiverId, content, callId) => {
//   try {
...
//           ongoingCalls.push(callPairing);
          
//           // Initialize chat session for this call
//           const chatKey = `${data.uniqueID}_${availableUser.uniqueID}`;
//           activeChats[chatKey] = {
//             messages: [],
//             participants: [data.uniqueID, availableUser.uniqueID]
//           };
...
//       // Add the chat_message event
//       case "chat_message":
//         console.log("new chat_message>>>");
//         console.log("data>>>", data);
        
//         // Find the ongoing call for this user
//         const userCall = ongoingCalls.find(
//           call => call.caller === data.uniqueID || call.receiver === data.uniqueID
//         );
        
//         if (userCall) {
//           // Determine the recipient
//           const recipientId = userCall.caller === data.uniqueID ? userCall.receiver : userCall.caller;
...
//             const chatKey = [data.uniqueID, recipientId].sort().join('_');
//             if (!activeChats[chatKey]) {
//               activeChats[chatKey] = {
//                 messages: [],
//                 participants: [data.uniqueID, recipientId]
//               };
//             }
            
//             // Add to in-memory chat storage
//             activeChats[chatKey].messages.push({
...
//         const currentUserCall = ongoingCalls.find(
//           call => call.caller === data.uniqueID || call.receiver === data.uniqueID
//         );
        
//         if (currentUserCall) {
//           const otherUserId = currentUserCall.caller === data.uniqueID ? 
//                              currentUserCall.receiver : currentUserCall.caller;
                             
//           // Get messages from database
//           try {
//             const messages = await Message.find({
//               $or: [
//                 { sender: data.uniqueID, receiver: otherUserId },
//                 { sender: otherUserId, receiver: data.uniqueID }
//               ]
...
//         // Find the call involving this user
//         const callToEnd = ongoingCalls.find(
//           call => call.caller === data.uniqueID || call.receiver === data.uniqueID
//         );
        
//         // Clean up chat state for this call
//         if (callToEnd) {
//           const chatKey1 = `${callToEnd.caller}_${callToEnd.receiver}`;
//           const chatKey2 = `${callToEnd.receiver}_${callToEnd.caller}`;
          
//           // Archive chat if needed before removing
//           if (activeChats[chatKey1]) {
//             delete activeChats[chatKey1];
//           } else if (activeChats[chatKey2]) {
//             delete activeChats[chatKey2];
//           }
//         }
...
//           const newCallPairing = {
//             caller: data.uniqueID,
//             receiver: nextAvailableUser.uniqueID,
//           };
          
//           ongoingCalls.push(newCallPairing);
          
//           // Initialize chat session for this new call
//           const newChatKey = `${data.uniqueID}_${nextAvailableUser.uniqueID}`;
//           activeChats[newChatKey] = {
//             messages: [],
//             participants: [data.uniqueID, nextAvailableUser.uniqueID]
//           };
...
//       // Find and end any calls involving this user
//       const callIndex = ongoingCalls.findIndex(
//         call => call.caller === disconnectedUser.uniqueID || call.receiver === disconnectedUser.uniqueID
//       );
...
//         const otherUserId = call.caller === disconnectedUser.uniqueID ? call.receiver : call.caller;
//         const otherUser = findUser(otherUserId);
        
//         // Notify the other user in the call that this user has disconnected
//         if (otherUser) {
//           io.to(otherUser.id).emit("message", {
//             type: "user_disconnected",
//             data: "The other user has disconnected",
//             disconnectedUser: disconnectedUser.uniqueID
//           });
//         }
...
//         const chatKey1 = `${call.caller}_${call.receiver}`;
//         const chatKey2 = `${call.receiver}_${call.caller}`;
...
// // Function to find a user by uniqueID
